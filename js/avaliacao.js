...
// INICIAR MODAL
export function iniciarAvaliacaoModal({ afterSave } = {}) {
  ...
  iniciarEstrelas()
  iniciarFechar()
  iniciarSalvar(afterSave)   // <<< PASSA O CALLBACK PRA FUNÇÃO DE SALVAR
  carregarLivros()
...
// SALVAR
function iniciarSalvar(afterSave) {
  const btn = document.getElementById('salvarAvaliacaoBtn')

  // sempre sobrescreve o onclick, evitando acumular vários listeners
  btn.onclick = async () => {
    ...
    // avisa a página de perfil pra recarregar a lista
